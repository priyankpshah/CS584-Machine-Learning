...
def Y_Testing(ztest,Ytest,Theta):
    ...
    for i in range (len(Y_predict)):
        for x in range(ztest.shape[1]):
            Y_predict[i] = Y_predict[i]+ (ztest[i,x]*Theta[x])
        Y_predictt = np.matrix(Y_predict).T
    Y_predict_sqr = np.square(Y_predictt - Ytest)
...
def Y_Training(ztrain,ytrain,Theta):
    ...
    for i in range (len(Y_predict)):
        for x in range(ztrain.shape[1]):
            Y_predict[i] = Y_predict[i]+ (ztrain[i,x]*Theta[x])
    Y_predictt = np.matrix(Y_predict).T
    error_train_sqr = np.square((Y_predictt - ytrain))
